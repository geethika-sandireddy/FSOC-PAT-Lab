...
class Gimbal:
    ...
    @staticmethod
    def _follow(cmd, now, vel, velmax, accmax, dt):
        """Damped position servo (PD) with slew/accel limits.

        a = kp*(cmd-now) - kd*vel   (critically damped, ~0.25s settle),
        then accel-clamped, velocity-clamped, position integrated.
        A pure velocity servo degenerates into bang-bang limit cycling
        around the target; the kd term gives the required damping.
        """
        err = cmd - now
        a = config.GIMBAL_SERVO_KP * err - config.GIMBAL_SERVO_KD * vel
